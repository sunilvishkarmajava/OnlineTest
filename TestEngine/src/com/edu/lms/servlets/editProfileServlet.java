...
//			userDAO userdao=new userDAO();
			...
			userdto.setRoleID(Integer.parseInt(session.getAttribute("role").toString()));
			userdto.setUserid(Integer.parseInt(session.getAttribute("loginid").toString()));
			
			try {
				ResultSet rs= userDAO.update(userdto);
				int result=0;
				while(rs.next()){
					result=rs.getInt(1);
				}
				...
				if(userdto.getRoleID()==1){
					...
					String page="Admineditprofile.jsp";
					//response.sendRedirect("register.jsp?error=Invalid User Id Or Password !");
					if(result>0){
						response.sendRedirect(page+"?status=value added succssfully !");
						//System.out.println("register successfully");
					}
					else if(result==-1){
						response.sendRedirect(page+"?status=user not exist !");
					}
					
					else if(result==0){
						response.sendRedirect(page+"?status=Server Error !");
					...
					String page="facultyeditprofile.jsp";
					//response.sendRedirect("register.jsp?error=Invalid User Id Or Password !");
					if(result>0){
						response.sendRedirect(page+"?status=value added succssfully !");
						//System.out.println("register successfully");
